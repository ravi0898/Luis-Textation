        ...
        <div class="col-md-12">
          <div class="tile">
            <h3 class="tile-title">Edit User</h3>
              ...
              @endif
            <div class="tile-body">
              <form method="POST" action="{{ route('add_users_list.update', $user->id ) }}">
                 @csrf
                  @method('PATCH')
                <div class="form-group">
                  <label class="control-label">Client Name:</label>
                  <input class="form-control" type="text" placeholder="Enter client name" name="client_name" id="client_name" value="{{ $user->client_name }}">
                </div>
                 <div class="form-group">
                    <label class="control-label">Company Name:</label>
                    <input class="form-control" type="text" placeholder="Enter company name" name="company_name" id="company_name" value="{{ $user->company_name }}">
                </div>
                <div class="form-group">
                  <label class="control-label">Email</label>
                  <input class="form-control" type="text" placeholder="Enter email address" name="email" id="email" value="{{ $user->email }}">
                </div>
                <div class="form-group">
                  <label class="control-label">Mobile No. :</label>
                  <input class="form-control" type="text" placeholder="Enter 10 digit mobile number" name="mobile_no" id="mobile_no" value="{{ $user->mobile_no }}">
                </div>
                <div class="form-group">
                  <label class="control-label">Priority :</label>
                  <select name="priority" id="priority" class="form-control">
                    <option {{ ($user->priority) == 'High' ? 'selected' : '' }}  value="High">High</option>
                    <option {{ ($user->priority) == 'Medium' ? 'selected' : '' }}  value="Medium">Medium</option>
                    <option {{ ($user->priority) == 'Low' ? 'selected' : '' }}  value="Low">Low</option> 
                </select>
                </div>
                
                <div class="tile-footer">
                  <button class="btn btn-primary" type="submit"><i class="fa fa-fw fa-lg fa-check-circle"></i>Update</button>&nbsp;&nbsp;&nbsp;<a class="btn btn-secondary" href="{{ url('listing') }}"><i class="fa fa-fw fa-lg fa-times-circle"></i>Cancel</a>
                </div>
              </form>
            </div>
